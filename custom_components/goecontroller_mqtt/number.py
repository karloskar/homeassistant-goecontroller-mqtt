...
from .definitions.number import NUMBERS, GoEControllerNumberEntityDescription
...
async def async_setup_entry(
    ...
    """Config entry setup."""
    async_add_entities(
        GoEControllerNumber(config_entry, description)
        for description in NUMBERS
        if not description.disabled
    )


class GoEControllerNumber(GoEControllerEntity, NumberEntity):
    """Representation of a go-eController switch that is updated via MQTT."""

    entity_description: GoEControllerNumberEntityDescription

    def __init__(
        self,
        config_entry: config_entries.ConfigEntry,
        description: GoEControllerNumberEntityDescription,
    ) -> None:
